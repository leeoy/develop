...
public class ClientActivity2 extends Activity {
	private static final String	TAG	= "张海南";
...
//		startService(startService);
	...
	//	serversText = (TextView)findViewById(R.id.clientServersText);
		...
		/*	transmit.setFilename("news.txt");
			transmit.setFilepath("/mnt/sdcard/download/news.txt");
			Intent sendDataIntent2 = new Intent(BluetoothTools.ACTION_DATA_TO_SERVICE);
			sendDataIntent2.putExtra(BluetoothTools.DATA, transmit);
			sendBroadcast(sendDataIntent2);    */
		}
		...
		sendBtn.setOnClickListener(new OnClickListener() {		
			...
			public void onClick(View v) {
				...
				if ("".equals(sendEditText.getText().toString().trim())) {
					Toast.makeText(ClientActivity2.this, "输入不能为空", Toast.LENGTH_SHORT).show();
				} else {
					//发送消息
		...
		filesendBtn.setOnClickListener(new OnClickListener() {		
			...
			public void onClick(View v) {
				...
				if ("".equals(mSendFileNameTV.getText().toString().trim())) {
					Toast.makeText(ClientActivity2.this, "未选择文件", Toast.LENGTH_SHORT).show();
				} else {
					TransmitBean transmit = new TransmitBean();
